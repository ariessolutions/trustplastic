...
@foreach ($supplier as $key => $supplier)
    ...
    <tr>
        ...
        <td class="py-1 align-middle">{{ $supplier->supplier_code }}</td>
        <td class="py-1 align-middle" style="max-width: 200px; overflow: hidden; text-overflow: ellipsis;  ">
            {{ $supplier->supplier_name }}</td>
        <td class="py-1 align-middle">{{ $supplier->supplier_primary_tel }}</td>
        ...
        @if ($supplier->status == 1)
            ...
            <td>
                <div class="input-group flex-nowrap">
                    <div class="m-1">
                        <a class="btn btn-secondary btn-sm" onclick="setSupplierValueForEdit({{ $supplier }});">
                            VIew / Edit
                        </a>
                    </div>
                    <div class="m-1">
                        <a class="btn btn-round btn-default btn-sm" data-id="form-control-{{ $supplier->id }}"
